;(function () {
  ...
  /**
   * @summary Check if the style element exists and that it has gradient-theme color content
  ...
  /**
   * @summary Checks if a theme is applied, and that the theme belongs to the gradient-theme family
   * @returns {boolean}
   */
  ...
  const readyForReplacement = (tokensEl, tokenReplacements) =>
    tokensEl
      ? // only init if we're using a gradient-theme subtheme
        usingSynthwave() &&
  ...
  const initNeonDreams = (disableGlow, obs) => {
    ...
    newStyleTag.setAttribute('id', 'gradient-theme-styles')
    newStyleTag.innerText = updatedThemeStyles.replace(/(\r\n|\n|\r)/gm, '')
    document.body.appendChild(newStyleTag)

    console.log('Gradient Theme: NEON DREAMS initialised!')
